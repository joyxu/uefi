...
EFI_STATUS OemBootOrderSetting(IN OUT UINT16* BootOrderFinal, UINTN BootOrderSize, BOOLEAN *IsFlashBootFirst, BOOLEAN *FlashBootSupport);
...
typedef enum
{
    CHAR_TYPE       = 0x0,   //变量中的值对应字符串
    NUMBER_RANGE    = 0x1,   //变量中的值对应实际的值
} SETUP_VARIABLE_TYPE;

typedef struct SETUP_STRING_INDEX
{
    UINT32 ulIndex;
    UINT8  ulSetupType;
    CHAR8  pName[0x40];
    CHAR8  (*pValue)[0x40];
}SETUP_STRING_INDEX_STRU;
...
EFI_STATUS GetSetupData(VOID *Setup);
...
VOID CpldRamWriteL3CacheBist(IN UINT32 SelfTestPara);
...
extern VOID FeedbackXGeStatus(BOOLEAN IsLinkup, BOOLEAN IsActOK, UINT32 port);
...
VOID OemDawConfigAsm(VOID);
VOID OemA57Config(VOID);
VOID OemGpioConfig(VOID);
VOID GICD_secondary_init_asm(VOID);
extern UINT8 gEthSetupDesc[];
