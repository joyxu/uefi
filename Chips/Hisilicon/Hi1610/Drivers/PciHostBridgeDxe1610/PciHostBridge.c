...
0I9NvvafLzjpuAt6wpO/8mXJIWeAypHfVItc8gGf0D9MU84Ok3KHSf3jx26tmLq73D+j5Uuw
sz7YUCnBOcToCLX1vDCh+k3OdBLAJtsPILvYV7/xlriq5kGv7GUMzQ07iyiV+g==#*/
...
//DTS2015081202801 将圈复杂度过高的函数拆分，降低圈复杂度 start>>
EFI_STATUS
EFIAPI
...
//DTS2015081202801 将圈复杂度过高的函数拆分，降低圈复杂度 end<<
...
**/
//DTS2015081202801 将Notify拆分，降低圈复杂度
EFI_STATUS
EFIAPI
NotifyPhase(
  ...
  while (List != &HostBridgeInstance->Head) {
    ...
    if (RootBridgeHandle == RootBridgeInstance->Handle) {
      while ( *Temp == 0x8A) {
        ...
        //  PCIE_DEBUG("Hide bridge resource\n");
        //  DEBUG((EFI_D_ERROR,"Hide bridge resource\n"));
        //  RootBridgeInstance->ResAllocNode[Index].Length = 0;
        //  RootBridgeInstance->ResAllocNode[Index].Status = ResNone;
        //} else {
